...
model = tf.keras.models.Sequential([
  tf.keras.layers.Dense(units=128, input_shape=[history_rolling.shape[1]-1], activation='relu', kernel_regularizer=tf.keras.regularizers.l2(l2=1e-4)),
    tf.keras.layers.Dense(32, activation='relu', kernel_regularizer=tf.keras.regularizers.l2(l2=1e-4)),
...
model.compile(loss="binary_crossentropy", optimizer=tf.keras.optimizers.Adam(learning_rate=1e-4),metrics=[tf.keras.metrics.AUC()])
...
for i in tqdm(range(len(Xy_rest_partitioned))):
    ...
    if use_logistic:
        ...
        history = model.fit(X_train_roll_shuffled_scaled, y_train_roll_shuffled, 
                    validation_data=(X_valid_roll_scaled, y_valid_roll),
...
# without Tacking_lag_1 feature
# rolling forecast
# F_beta score (beta=1):  0.022354819732750653
# AUC score:  0.49830803025247467
